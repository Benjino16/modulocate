...
export interface AllocationConfig {
  ...
  seed: number;
  // When true (the default — omit to get it), allocate() orders each
  // student's unranked-module tail (see allocate.ts's
  // orderByFillPriorityThenShuffle) by fill priority instead of pure
  // randomization: modules still below their (real, live) min go first,
  // ranked by an internal dry run's *predicted* deficit rather than raw min
  // (so a big, genuinely popular module doesn't outrank a small,
  // undersubscribed one just because its min is numerically larger); once a
  // module's live assignedTotal reaches its min, modules are ordered by the
  // largest fraction of max still free right now. This steers unranked/filler
  // assignments toward modules that actually need seats to stay viable
  // (below_min_capacity) before merely topping up already-viable ones, while
  // ties within either group are still randomized so no id-bias reappears.
  // Set false to fall back to plain per-student randomization (and to skip
  // the extra dry-run cost).
  fillAwareUnrankedOrder?: boolean;
}
