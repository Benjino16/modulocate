...
- **Node.js 22** (see `.nvmrc`) — only needed locally for your editor's TypeScript/ESLint tooling and to run `pnpm`. The apps themselves run inside Docker, not on your host.
...
Once the `backend` container is up, apply the database schema (run on your host — `packages/db`'s `.env` values point at `localhost`, which works because Postgres's port is published to the host):

```bash
pnpm --filter @modulocate/db db:migrate
pnpm --filter @modulocate/db db:seed   # optional: sample data
```
...
pnpm --filter @modulocate/db db:studio            # browse the DB with Drizzle Studio
