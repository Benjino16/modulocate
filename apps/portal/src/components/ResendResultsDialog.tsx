...
export function ResendResultsDialog({
  projectId,
  student,
  open,
  onOpenChange,
  ...
  function handleConfirm() {
    if (!student) return;
    setError(undefined);
    sendVotingResults.mutate({ projectId, studentIds: [student.id] });
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Ergebnisse erneut zusenden</DialogTitle>
        </DialogHeader>
        ...
        <p className="text-sm">Möchtest du dem Schüler die Ergebnisse erneut zusenden?</p>
        ...
        <DialogFooter>
          <Button
            ...
            disabled={sendVotingResults.isPending}
          >
            Abbrechen
          </Button>
          <Button type="button" onClick={handleConfirm} disabled={sendVotingResults.isPending}>
            Bestätigen
          </Button>
        </DialogFooter>
      </DialogContent>
