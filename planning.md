...
- **CORS/trustedOrigins:** `trustedOrigins: ["http://*:5173", "http://*:5174"]` in `auth.ts` mirrors the existing dev-only CORS regex in `index.ts` (any host on port 5173/5174, for LAN/phone testing) — same "tighten before production" caveat as the existing CORS note in Section 6 applies here too.
- **Portal tRPC client needs `credentials: "include"` explicitly** (`apps/portal/src/trpc.ts`, passed via `httpBatchLink`'s `fetch` option). Discovered the hard way: after login, the portal could reach `/api/auth/get-session` (better-auth's own client sets `credentials: "include"` internally) but every `staffProcedure` call still 401'd, because `fetch()` defaults to `credentials: "same-origin"` and portal (`:5173`) → backend (`:3000`) counts as cross-origin. `curl` testing during development didn't catch this — `curl -b cookies.txt` always sends cookies regardless of origin, so the gap only showed up once tested in an actual browser. Same underlying cross-origin-cookie fact as the CORS `credentials: true` note above, just the client-side half of it.
