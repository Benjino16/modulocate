...
- **Flow:** the vote app calls a login mutation with the `signInCode` from the emailed link; the backend looks up the student, signs a JWT (`studentId`, `projectId`, `exp`), and sets it as an `HttpOnly; SameSite=Lax` cookie. `vote-web` and `backend` are same-origin behind Traefik (`http://modulocate.localhost`, path-routed — see the Docker Compose backlog item in Section 6), so no cross-origin cookie/CORS handling is needed at all, in dev or prod. Subsequent vote routes go through a `protectedStudentProcedure` that verifies the cookie and attaches `ctx.student`. Logout is a mutation that clears the cookie server-side (can't be done client-side, since it's `HttpOnly`). `signInCode` itself never expires — re-clicking the email link always mints a fresh cookie, which doubles as "lost session" recovery without extra work.
...
### Locked Decision: Staff Auth Ships Flat (No Roles Yet), Gates Every Portal Route from Day One

better-auth (email/password only, no OAuth/2FA plugins) is wired up in `apps/backend/src/auth.ts` and gates every portal-facing router (`projects`, `students`, `modules`, `moduleCategories`, `dates`, `rules`, `studentGroups`, `mail`, `allocationRuns`) via a single new `staffProcedure` in `trpc.ts` — replacing `publicProcedure` on all of them in one pass rather than rolling it out router-by-router. `vote`/`voteAuth` are untouched (still the student JWT-cookie flow above).

- **No roles/permissions distinction yet** — any signed-in staff user can call every staff-facing procedure. The "admin vs. teacher, may teachers only see their own modules" question from Section 6 stays open and unimplemented; this only answers "is anyone signed in at all." Deliberate: better-auth's plugin system (e.g. the `admin` plugin, or a plain `role` field via `additionalFields`) makes adding roles later cheap, so there was no need to design that model before shipping the base layer. If per-teacher module ownership lands, it needs a schema change (`ownerId`/`createdBy` on `modules`) independent of the auth plugin choice — a data-model question, not an auth-library one.
- **`users`/`sessions`/`accounts`/`verifications` tables** replace the old hand-rolled placeholder `users` table (`username`/`email`/`passwordHash`, never actually wired to any auth logic). Table/column naming mirrors better-auth's core schema but stays plural (`usePlural: true` on the drizzle adapter) to match this codebase's convention (`projects`, `students`, `modules`, ...). Ids are `text`, not `uuid()` — better-auth generates/compares ids as plain strings — but `auth.ts`'s `advanced.database.generateId` fills them with `crypto.randomUUID()` so values still look like every other table's uuid PK.
- **Fastify integration:** better-auth's fetch-`Request`-based handler is bridged onto Fastify at `/api/auth/*` in `index.ts` by reconstructing a `Request` from Fastify's already-parsed `request.body`/headers (not by reading `request.raw` a second time, which would find the stream already drained by Fastify's default JSON content-type parser).
- **Bootstrap admin account** (`apps/backend/src/bootstrapAdmin.ts`): if `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_NAME` are all set and no user with that email exists yet, one is created via `auth.api.signUpEmail` on backend startup — the classic "seed a superuser from env" pattern, so a fresh dev/staging environment always has one usable account without a manual signup step. No-op (not an error) if the vars are unset.
- **Portal login page** (`apps/portal/src/routes/login.tsx` + `src/lib/auth-client.ts`) is intentionally minimal — email/password form only, no signup/invite UI. `__root.tsx`'s `AuthGuard` redirects to `/login` whenever `authClient.useSession()` has no session, for every route except `/login` itself. The fuller account-management UI (inviting/creating other staff accounts, `settings.account-management.tsx`) is explicitly deferred to a later pass, once roles exist to decide who's allowed to invite whom.
- **No CORS/trustedOrigins needed:** portal and backend are same-origin behind Traefik (`http://modulocate.localhost`, path-routed to `/portal` and `/api` — see the Docker Compose item in Section 6), so `auth.ts` has no `trustedOrigins` and `index.ts` has no CORS plugin at all. This used to require a dev-only CORS regex + `trustedOrigins` wildcard matching ports 5173/5174 (back when portal/backend ran as separate host processes on different ports) plus an explicit `credentials: "include"` override in the portal's tRPC client (`fetch()` defaults to `credentials: "same-origin"`, which drops cookies cross-origin) — all of that is gone now that there's no cross-origin request in the first place, in dev or prod.
- **Schema written by hand, then verified against the better-auth CLI, not auto-synced.** `users`/`sessions`/`accounts`/`verifications` in `schema.ts` were typed out manually (to land directly in the single shared `schema.ts` file, matching this codebase's one-file-schema convention) rather than by running `@better-auth/cli generate`, which by default writes a separate file. Ran the generator retroactively as a check (`npx @better-auth/cli generate --config apps/backend/src/auth.ts`) and diffed it against the hand-written version: field names/types/constraints matched, but the generator additionally adds indexes on FK/lookup columns (`sessions.userId`, `accounts.userId`, `verifications.identifier`) and `.$onUpdate(() => new Date())` on every `updatedAt` column — both now added to `schema.ts` to match. `{ withTimezone: true }` on every timestamp is a deliberate, permanent deviation from the generator's plain `timestamp` output, to match every other timestamp column in this file.
  - **Going forward:** whenever a better-auth plugin is added that changes required fields/tables (e.g. the `admin` plugin adding `role`/`banned` to `users`, or a 2FA plugin adding its own table), re-run the generator and diff it against `schema.ts` by hand — same rhythm as `drizzle-kit generate` for migrations, a dev-time sync step, not something better-auth keeps in sync automatically at runtime.

### Locked Decision: Single Root `.env`, No `dotenv` in App Code

Previously `apps/backend`, `apps/worker`, and `packages/db` each had their own `.env`/`.env.example`, loaded via `import "dotenv/config"`, with `DATABASE_URL`/`REDIS_URL`/`SMTP_*` duplicated across all three (and `infra/compose.yaml` already overrode the Docker-relevant ones anyway — `DATABASE_URL`/`REDIS_URL`/`SMTP_HOST`, via its own `environment:` block — so those particular duplicated values were already dead for the Docker path). Replaced with one `.env`/`.env.example` at the repo root:

- **Why root-level, not per-app:** every service here is one deploy unit (one Compose stack), not independently deployable with separate secrets — per-app `.env` files bought no isolation, only duplication to keep in sync. A single file is also where a developer expects to find it, and matches the eventual production layout (a `.env` next to whichever `compose.*.yaml` is active on the server, populated by the deploy process rather than hand-copied from an app subfolder).
- **Why `dotenv` came out of `apps/backend/src/index.ts`, `apps/worker/src/index.ts`, and `packages/db/src/client.ts`:** `process.env` is already the real process environment — no library needed to read it. `dotenv` only earns its keep when *nothing else* populates `process.env` before the process starts. For `backend`/`worker`, Docker Compose's `env_file: ../.env` already injects every variable into the container before Node runs, making `dotenv/config` redundant there. `packages/db`'s CLI scripts (`db:generate`/`db:migrate`/`db:seed`/`db:studio`) previously ran on the host via `pnpm --filter @modulocate/db ...` against the Postgres port Compose publishes to `localhost` — the one place `dotenv` was actually load-bearing. Fixed by running them inside a container instead (same image as `backend`, already has the full workspace installed), e.g.:
  ```
  docker compose -f infra/compose.yaml run --rm backend pnpm --filter @modulocate/db db:migrate
  ```
  which resolves `postgres:5432` on the Compose network and gets `DATABASE_URL` etc. from the same `env_file` mechanism as `backend`/`worker` — no separate `.env` for `packages/db`, no `dotenv` dependency anywhere in the workspace.
- **Migrations specifically run as a one-off container invocation, not baked into a service's startup command:** avoids multiple app replicas racing to migrate concurrently, and matches how this will work in production once pre-built images replace `build:` in a prod Compose file — a discrete migration step at deploy time, not app-container boot logic.
...
- [ ] Refine the roles/permissions model (admin vs. teacher — may teachers only see/edit their own modules?). Base staff auth (better-auth, no roles, gates every portal route) now exists — see "Locked Decision: Staff Auth Ships Flat" above — this item is now specifically about the role/ownership layer on top of it, not about auth existing at all.
- [x] Set up the Docker Compose setup for local development (Postgres, Redis, backend, worker, portal, vote — MinIO not included yet, only S3 env vars). Went further than originally scoped here: all four apps run behind a Traefik reverse proxy (`http://modulocate.localhost`, path-routed to `/api`, `/portal`, `/voting`), so backend/portal/vote are same-origin in dev — see the updated CORS/trustedOrigins prose under "Locked Decision: Staff Auth Ships Flat" above. `infra/compose.yaml` (base) + `infra/compose.dev.yaml` (bind-mount overlay for hot reload) — `pnpm dev` now runs this instead of `turbo run dev`. Production is intended to run behind Traefik with this same path layout; the actual production image (multi-stage build, compiled output) is still a separate future task.
...
- [ ] **Revisit dev-mode CORS/cookie relaxations before any move toward production.** To unblock local mobile testing (frontend/backend on different LAN ports/hosts), the backend CORS origin check (`apps/backend/src/index.ts`) was widened to a regex matching port 5173/5174 on *any* host, and the student session cookie (`apps/backend/src/routers/voteAuth.ts`) was switched from `secure: true, sameSite: "none"` to `secure: false, sameSite: "lax"`. Both are correct for same-host, plain-HTTP dev (frontend/backend only differ by port, which browsers treat as same-site) but must be tightened again — explicit origin allowlist, `secure: true` + `sameSite: "none"` behind real HTTPS — once frontend and backend are no longer on the same host/served over plain HTTP.
